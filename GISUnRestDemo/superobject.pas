...
var
 SO_CaseSensitive : Boolean = true;     //  для поиска идентификаторов с учетом или без учета регистра символов     vadim  10.02.2022
...
begin
  h := 0;
  //--  vadim   -----
  if SO_CaseSensitive then begin      // default  
    for i := 1 to Length(k) do
        h := h*129 + ord(k[i]) + $9e370001;
  end else begin
    k1:=WideUpperCase(k);            //  для поиска без учета регистра
    for i := 1 to Length(k1) do
      h := h*129 + ord(k1[i]) + $9e370001;
  end;
  Result := h;
  //-----------------
// было до изменения  10.02.2022
//  h := 0;
//  for i := 1 to Length(k) do
//      h := h*129 + ord(k[i]) + $9e370001;
//  Result := h;
